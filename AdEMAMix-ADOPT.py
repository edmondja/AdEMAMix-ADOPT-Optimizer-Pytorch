...
class AdEMAMix(Optimizer):
    ...
    def _update_adamemix(self, params, grads, exp_avgs, exp_avg_sqs, exp_avg_slow, state_steps,
        ...
        for i, param in enumerate(params):
            ...
            # Decay the first and second moment running average coefficient if torch.min(exp_avg_sq) == 0:
            if torch.min(exp_avg_sq) == 0:
                sq_norm = torch.ones_like(grad)
            else:
                sq_norm = torch.maximum(torch.sqrt(exp_avg_sq), torch.tensor(eps * 100))
            exp_avg.mul_(beta1).addcdiv_(grad, sq_norm, value=1 - beta1)
            exp_avg_slow_i.mul_(beta3_t).addcdiv_(grad, sq_norm, value=1 - beta3_t)
